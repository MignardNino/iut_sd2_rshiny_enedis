         ...
         #  "corrplot", "leaflet", "htmlwidgets", "DT", "shinymanager","sf"))
              ...
              get_data_69_existants <- function() {
                ...
                while (has_more_data) {
                  ...
                  # Encoder l'URL avec les paramètres
                  ...
                  if (status_code(response) == 200) {
                    ...
                    # Arrêter la boucle si moins de données ont été récupérées que la taille de page définie
              ...
              get_data_69_neufs <- function() {
                ...
                page_size <- 1000
                has_more_data <- TRUE
                # Boucle pour récupérer toutes les pages de données et Définir les paramètres de la requête
                while (has_more_data) {
                  params <- list(
                    page = page,
                    size = page_size,
              ...
              credentials <- data.frame(
                user = c("shiny", "shinymanager","asardell"), # user
                password = c("azerty", "12345","SD2024"), # mdp
                admin = c(FALSE, TRUE,FALSE), #oui/non admin
              ...
              ui <- fluidPage(
                ...
                # Lien dynamique vers le fichier CSS sélectionné
                tags$head(
                  uiOutput("css_link")
                ),
                
                titlePanel("Application DPE - Analyse des logements existants et neufs (69)"),
                # Mise en page avec une barre latérale
                sidebarLayout(
                  sidebarPanel(
                  ...
                  # Différentes sections de l'application organisées en onglets
                  mainPanel(
                    tabsetPanel(
              ...
              server <- function(input, output, session) {
                # Fonction pour sécuriser l'accès à l'application
